...
void CtrTask_Entry(void const * argument)
{
  ...
  PID_struct_init(&pid_pit,POSITION_PID,30,0,2,0,0);//参数放大到10000倍，方便调整 
  ...
  for(;;)
  {
    ...
    pid_speed_out_left   = pid_calc(&pid_speed, Motors.Motor3.Speed, -motor_ctrl.forward_speed);//3->left,1->right 正负不确定
    pid_speed_out_right   = pid_calc(&pid_speed, -Motors.Motor1.Speed, -motor_ctrl.forward_speed);//3->left,1->right 正负不确定
...
void motor_ctrl_para_init(motor_ctrl_t *motor_ctrl)
{
  ...
  motor_ctrl->motor_start_flag = 0;//1:启动 0:停止
  motor_ctrl->motor_safe_flag = 1;//1:安全 0:不安全
}
